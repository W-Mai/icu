...
APP_NAME="icu_tool"
APP_VERSION="0.10.0"
if [ -n "${ICU_TOOL_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$ICU_TOOL_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${ICU_TOOL_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$ICU_TOOL_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/W-Mai/icu/releases/download/v0.10.0"
elif [ -n "${ICU_TOOL_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$ICU_TOOL_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/W-Mai/icu/releases/download/v0.10.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/W-Mai/icu/releases/download/v0.10.0"
fi
if [ -n "${ICU_TOOL_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$ICU_TOOL_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${ICU_TOOL_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$ICU_TOOL_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${ICU_TOOL_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$ICU_TOOL_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${ICU_TOOL_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${ICU_TOOL_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${ICU_TOOL_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"icu_tool","name":"icu","owner":"W-Mai","release_type":"github"},"version":"0.10.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/icu_tool"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/icu_tool"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
icu_tool-installer.sh

The installer for icu_tool 0.10.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/W-Mai/icu/releases/download/v0.10.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set ICU_TOOL_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "icu_tool-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="bc6a5393b2bccaa50403fa6cee7b0e986ccfa628b4380f5abb6289aa9c969760"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-aarch64-unknown-linux-gnu.tar.xz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="eec8d3e6ff9fc613c077175d0ef95b29393ef39195985e7118f75b2570f39406"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-aarch64-unknown-linux-musl.tar.xz")
            _arch="aarch64-unknown-linux-musl-static"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="894e2adc72f3f59629ef1e4ed3ac1761bcd2d23b70ffeef962073d0baef236fc"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-x86_64-apple-darwin.tar.xz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="b703f0727709253843cd24fb8b7d7e3663d2ba75b96aa2d1b9944a702b343add"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="c2afe9418de14448aaa99c031a8917395864b10cee8e8ac10aebb59d141ecf28"
            _bins="icu.exe"
            _bins_js_array='"icu.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-x86_64-unknown-linux-gnu.tar.xz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="0e2dfcab93a2f4df5039e8c2b3e9dd2e254846bcbbb2d6f9956f7484698825e4"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "icu_tool-x86_64-unknown-linux-musl.tar.xz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="82f4aa9d5ddf14f7b9ba19b0a474ee9efe4ec1668474701d536958b71230398a"
            _bins="icu"
            _bins_js_array='"icu"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="icu_tool-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="icu_tool-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="icu_tool-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="icu_tool-aarch64-unknown-linux-gnu.tar.xz"
            ...
            _archive="icu_tool-aarch64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="icu_tool-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "35"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="icu_tool-x86_64-unknown-linux-musl.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${ICU_TOOL_INSTALL_DIR:-}" ]; then
        _force_install_dir="$ICU_TOOL_INSTALL_DIR"
