use eframe::egui;
use eframe::egui::{RichText, StrokeKind};

#[allow(dead_code)]
fn toggle_ui(ui: &mut egui::Ui, label: impl Into<RichText>, on: &mut bool) -> egui::Response {
    ui.horizontal(|ui| {
        let desired_size = ui.spacing().interact_size.y * egui::vec2(2.0, 1.0);
        let (rect, mut response) = ui.allocate_exact_size(desired_size, egui::Sense::click());
        if response.clicked() {
            *on = !*on;
            response.mark_changed();
        }
        response.widget_info(|| {
            egui::WidgetInfo::selected(egui::WidgetType::Checkbox, true, *on, "")
        });

        if ui.is_rect_visible(rect) {
            let how_on = ui.ctx().animate_bool(response.id, *on);
            let visuals = ui.style().interact_selectable(&response, *on);
            let rect = rect.expand(visuals.expansion);
            let radius = 0.5 * rect.height();
            ui.painter()
                .rect(rect, radius, visuals.bg_fill, visuals.bg_stroke, StrokeKind::Outside);
            let circle_x = egui::lerp((rect.left() + radius)..=(rect.right() - radius), how_on);
            let center = egui::pos2(circle_x, rect.center().y);
            ui.painter()
                .circle(center, 0.75 * radius, visuals.bg_fill, visuals.fg_stroke);
        }

        let label_response = ui.add(egui::Label::new(label.into()).sense(egui::Sense::click()));
        if label_response.clicked() {
            *on = !*on;
            response.mark_changed();
        }

        response | label_response
    })
    .inner
}

pub fn toggle<'a>(label: impl Into<RichText> + 'a, on: &'a mut bool) -> impl egui::Widget + 'a {
    move |ui: &mut egui::Ui| toggle_ui(ui, label, on)
}
